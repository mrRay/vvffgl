...
- (id)initWithPlugin:(FFGLPlugin *)plugin context:(FFGLContext *)context outputHint:(FFGLRendererHint)hint
{
    if (self = [super init])
	{
        if ([self class] == [FFGLRenderer class])
		{
			...
			{
                return [[FFGLGPURenderer alloc] initWithPlugin:plugin context:context outputHint:hint];
            }
			else if ([plugin mode] == FFGLPluginModeCPU)
			{
                return [[FFGLCPURenderer alloc] initWithPlugin:plugin context:context outputHint:hint];
            } 
			else
			{
                return nil;
		...
		else
		{
            if ((plugin == nil)
                || (([plugin mode] == FFGLPluginModeCPU)
                    && ![[plugin supportedBufferPixelFormats] containsObject:[context pixelFormat]])
            ...
            _instance = [plugin _newInstanceWithSize:[context size] pixelFormat:[context pixelFormat]];
