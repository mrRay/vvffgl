...
- (IBAction)addRendererFromTableView:(NSTableView *)sender
{
    ...
    if ((selectedRow >= 0) && (selectedRow < [sourceArray count])) {
		if (_context == nil)
		{
			_context = [[FFGLContext alloc] initWithCGLContext:[[_renderView openGLContext] CGLContextObj] pixelFormat:kFFPixelFormat size:kRenderDimensions];
		}
